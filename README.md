...
Почти всегда причина одна: **в `client.yaml` нет полей `country` и `country_code`.**
Это единственный источник этих данных — приложения намеренно не определяют страну по
IP сервера. Раньше определяли, через сторонний геосервис, и это по таймеру сдавало ему
адрес вашего сервера; функцию убрали.

Добавьте в конфиг и сохраните его в приложении заново:
...
`country_code` — двухбуквенный код ISO. На Android из него локально собирается эмодзи
флага (🇳🇱), ничего не скачивается. На Windows показывается только текст: Chromium под
Windows флаговые эмодзи не отрисовывает — это ограничение платформы, а не приложения.

Метка записывается сразу при сохранении конфигурации и закрепляется; интернет для
этого не нужен. Если поля в конфиге есть, а метка не появилась — обновите приложение:
в сборках **до 1.11.0** страна терялась, если в момент сохранения не проходил пинг до
сервера, и больше не восстанавливалась. Новая версия дополнительно чинит уже
сохранённые конфигурации при первом запуске.

IP на плитке — величина отдельная: он определяется пингом до **вашего** сервера и
сеть требует. Если сети нет, приложение продолжит попытки с нарастающей паузой, пока
не получится, и запомнит результат.

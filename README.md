...
- **Desktop-прокси (Windows/Linux)** — `client.exe`/`client-linux`, локальный SOCKS5 +
  HTTP CONNECT прокси, плюс Windows-менеджер `vpn.exe`. Не требует прав администратора,
  но туннелирует только явно настроенные на прокси приложения.
...
Android и Windows-приложение делят один и тот же netstack-движок
([internal/netstack](internal/netstack)) поверх gVisor — платформенная разница только в
том, как пакеты попадают в этот движок (файловый дескриптор `VpnService` на Android,
`golang.zx2c4.com/wireguard/tun`+Wintun на Windows).

...
#   PHANTOM_PORT      port the VPN listens on (default 8443)
#   PHANTOM_APP=1     run on behalf of the Phantom apps ("Подключить свой сервер"): never
#                     prompts, and prints "@@PHANTOM ..." marker lines - steps, an error code
#                     on failure, and the finished client.yaml between client-begin/client-end -
#                     for the app to read instead of scraping the human-readable log.
#
...
# mark <words...>: a machine-readable line for the apps (PHANTOM_APP=1 only).
mark() { if [ "${PHANTOM_APP:-}" = "1" ]; then echo "@@PHANTOM $*"; fi; }
# die <message> [code]: the code is what the apps show a translated message for.
die()  { mark "error ${2:-failed}"; printf '\033[1;31mERROR:\033[0m %s\n' "$1" >&2; exit 1; }

require_root() {
    [ "$(id -u)" = "0" ] || die "must be run as root (try: curl ... | sudo sh)" not_root
}
...
detect_arch() {
    case "$(uname -m)" in
        ...
        *) die "unsupported architecture: $(uname -m)" unsupported_arch ;;
    esac
}
...
require_root
mark step checks
# Minimal VPS images sometimes ship without curl; install it rather than stop
# someone who only has the login and password their hosting sent them.
if ! command -v curl >/dev/null 2>&1; then
    log "curl not found - installing it..."
    if command -v apt-get >/dev/null 2>&1; then
        DEBIAN_FRONTEND=noninteractive apt-get update -qq >/dev/null 2>&1 || true
        DEBIAN_FRONTEND=noninteractive apt-get install -y -qq curl ca-certificates >/dev/null 2>&1 || true
    elif command -v dnf >/dev/null 2>&1; then
        dnf install -y -q curl >/dev/null 2>&1 || true
    elif command -v yum >/dev/null 2>&1; then
        yum install -y -q curl >/dev/null 2>&1 || true
    fi
    command -v curl >/dev/null 2>&1 || die "curl is required" no_curl
fi
command -v systemctl >/dev/null 2>&1 || die "systemd is required (systemctl not found)" no_systemd

PORT="${PHANTOM_PORT:-8443}"
case "$PORT" in
    ''|*[!0-9]*) die "PHANTOM_PORT must be a number, got '$PORT'" bad_port ;;
esac
[ "$PORT" -ge 1 ] && [ "$PORT" -le 65535 ] || die "PHANTOM_PORT out of range: $PORT" bad_port
[ "$PORT" -ne 80 ] || die "port 80 is needed for the certificate - pick another PHANTOM_PORT" bad_port
...
verify_checksum() {
    ...
    if [ "$_got" != "$_want" ]; then
        rm -f "$_file"
        die "checksum mismatch for $_name (expected $_want, got $_got) - refusing to install" checksum
    fi
...
curl -fsSL "$BASE_URL/phantom-server-linux-$ARCH" -o "$INSTALL_DIR/phantom-server.new" \
    || die "could not download phantom-server from $BASE_URL" download_failed
...
[ -n "$DOMAIN" ] || die "no domain given - set PHANTOM_DOMAIN=yourdomain.example and re-run, or run this interactively over SSH" no_domain
...
if [ -n "$CERT_FILE" ] || [ -n "$KEY_FILE" ]; then
    ...
    [ -f "$CERT_FILE" ] || die "cert file not found: $CERT_FILE" cert_missing
    [ -f "$KEY_FILE" ] || die "key file not found: $KEY_FILE" cert_missing
    log "Using existing certificate $CERT_FILE - Phantom's own ACME/port 80 responder will stay off."
fi

# Ports: taken by something else, the service would start and then fail to
# bind, or never get its certificate - say so now, by name.
if command -v ss >/dev/null 2>&1; then
    if [ -n "$(ss -ltnH "sport = :$PORT" 2>/dev/null)" ]; then
        die "port $PORT is already in use on this server - pick another PHANTOM_PORT" port_busy
    fi
    if [ -z "$CERT_FILE" ] && [ -n "$(ss -ltnH "sport = :80" 2>/dev/null)" ]; then
        die "port 80 is already in use - Let's Encrypt needs it to issue the certificate" port80_busy
    fi
fi
...
curl -fsSL "$BASE_URL/phantom-keygen-linux-$ARCH" -o /tmp/phantom-keygen \
    || die "could not download phantom-keygen from $BASE_URL" download_failed
...
[ -n "$SERVER_PRIV" ] && [ -n "$SERVER_PUB" ] && [ -n "$PSK" ] || die "failed to parse keygen output" keygen
...
if command -v firewall-cmd >/dev/null 2>&1 && firewall-cmd --state >/dev/null 2>&1; then
    [ -z "$CERT_FILE" ] && { firewall-cmd --permanent --add-port=80/tcp >/dev/null 2>&1 || true; }
...
if ! systemctl is-active --quiet "$SERVICE_NAME"; then
    ...
    die "Service did not start - check: journalctl -u $SERVICE_NAME -n 50" service_failed
fi

CLIENT_YAML="server: \"$DOMAIN:$PORT\"
domain: \"$DOMAIN\"
fingerprint: \"auto\"
psk: \"$PSK\"
